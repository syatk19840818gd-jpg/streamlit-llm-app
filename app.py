...
[②] その中から、小学生が興味を持ちそうな内容を２つ選ぶ（不確かな情報や、マイナーすぎる情報は除外する）。
[③] ②から問題文と解答をセットでつくる。
...
・漢字を少なくする（小学3年生でも読めるレベル）。難しい漢字はひらがなにする。
・「画数が多くて線の密度がある漢字」は ひらがな/カタカナ にする。
・question/answer/explanationの中で " は使わない（強調は「」）

【重要：問題作成の禁止・制限事項】
・「一番」「最古」などの順位を扱う場合は、必ず「日本で一番」「世界で一番」「道路トンネルの中で」のように**範囲や条件を問題文に明記**すること。
