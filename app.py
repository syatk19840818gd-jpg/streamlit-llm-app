...
【重要：クイズの質のルール（ここを守らないとクイズになりません）】
・**答えが「{topic}」そのものになる問題は絶対に作らないこと。**
  （ダメな例：「{topic}」がパンダの場合、「白黒の動物は？」→答え「パンダ」）
・**定義を聞くのではなく、「中身」を聞くこと。**
  （良い例：「{topic}」がパンダの場合、「パンダが食べる草は？」→答え「ささ」）
・答えは必ず「名詞（モノの名前）」にすること。
...
EXAMPLE_TEXTS = {
    "teach": "たとえば①「パンダ」「アメリカ」「音楽」「うちゅう」など<br>たとえば②「なんで〇〇は〇〇なの？」など",
